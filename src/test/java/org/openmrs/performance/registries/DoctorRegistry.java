...
public class DoctorRegistry extends Registry<DoctorHttpService>{
	...
	public ChainBuilder startVisit(String patientUuid) {
		...
		return exec(httpService.getVisitTypes())
				.exec(httpService.getCurrentVisit(patientUuid))
				.exec(httpService.getVisitsOfPatient(patientUuid))
				.exec(httpService.getProgramEnrollments(patientUuid))
				.exec(httpService.getVisitQueueEntry(patientUuid))
				.exec(httpService.getAppointments(patientUuid))
				.pause(5)
	...
	public ChainBuilder openOrdersTab(String patientUuid) {
		return exec(httpService.getOrderTypes())
				.exec(httpService.getAllActiveOrders(patientUuid));
	}
	
	public ChainBuilder openLabResultsTab(String patientUuid) {
		return exec(httpService.getLabResults(patientUuid))
				.exec(session -> {
				...
				.foreach("#{labResultConceptIds}", "conceptId").on(
						exec(httpService.getConcept("#{conceptId}"))
				);
	}
	...
	public ChainBuilder openAttachmentsTab(String patientUuid) {
		return exec(httpService.getAttachments(patientUuid))
	...
	public ChainBuilder addVisitNote(String patientUuid, String currentUserUuid) {
		...
		String diabeticKetosisConcept = "123107AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
		String diabeticFootUlcerConcept = "149069AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA";
